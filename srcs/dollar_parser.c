...
char	*replace_last_command(char *line, int dollar_pos, int end)
{
	char	*replacement;
	char	*to_replace;
	int		final_len;
	...
	if (!(to_replace = ft_strsub(line, dollar_pos, end - dollar_pos)))
		exit_func(MERROR);
	if (!(replacement = get_var(to_replace + 1)))
		replacement = "";
	free(to_replace);
	replacement_len = ft_strlen(replacement);
	final_len = dollar_pos + replacement_len + ft_strlen(line) - end + 1;
...
char	*search_dollar(char *line)
{
	...
	while ((dollar_pos = ft_indexof(line, '$')) != -1)
	{
		...
		if (line[end] == '?')
		{
			...
			tmp = replace_last_command(line, dollar_pos, end);
		}
		else if (end != dollar_pos + 1)
